...
public class DataView {
	...
	//		Log.i("check", String.valueOf(po));
			...
			if (mixContext.getStartUrl().length() > 0)
			{
				...
				switch(po)
				{
					...
					case 1:
						...
						isLauncherStarted = true; break;	// 런쳐는 시작된 상태로
					case 2:
			...
			// URL 이 할당되지 않았을 경우에는
				...
				if(po==1){
					if(mixContext.isDataSourceSelected(DataSource.DATASOURCE.CONVENIENCE)){
						requestData(DataSource.createRequestURL(DataSource.DATASOURCE.CONVENIENCE, lat, lon, alt, radius, Locale.getDefault().getLanguage(), searchlat, searchlon),
						DataSource.dataFormatFromDataSource(DataSource.DATASOURCE.CONVENIENCE),DataSource.DATASOURCE.CONVENIENCE);}		
				}
				else if(po==2){
					if(mixContext.isDataSourceSelected(DataSource.DATASOURCE.CAFE)){
						requestData(DataSource.createRequestURL(DataSource.DATASOURCE.CAFE, lat, lon, alt, radius, Locale.getDefault().getLanguage(), searchlat, searchlon),
					...
					if(mixContext.isDataSourceSelected(DataSource.DATASOURCE.BANK)){
						requestData(DataSource.createRequestURL(DataSource.DATASOURCE.BANK, lat, lon, alt, radius, Locale.getDefault().getLanguage(), searchlat, searchlon),
					...
					if(mixContext.isDataSourceSelected(DataSource.DATASOURCE.PCCAFE)){
						requestData(DataSource.createRequestURL(DataSource.DATASOURCE.PCCAFE, lat, lon, alt, radius, Locale.getDefault().getLanguage(), searchlat, searchlon),
					...
					if(mixContext.isDataSourceSelected(DataSource.DATASOURCE.HOSPITAL)){
						requestData(DataSource.createRequestURL(DataSource.DATASOURCE.HOSPITAL, lat, lon, alt, radius, Locale.getDefault().getLanguage(), searchlat, searchlon),
					...
					if(mixContext.isDataSourceSelected(DataSource.DATASOURCE.PHARMACY)){
						requestData(DataSource.createRequestURL(DataSource.DATASOURCE.PHARMACY, lat, lon, alt, radius, Locale.getDefault().getLanguage(), searchlat, searchlon),
					...
					if(mixContext.isDataSourceSelected(DataSource.DATASOURCE.HOTEL)){
						requestData(DataSource.createRequestURL(DataSource.DATASOURCE.HOTEL, lat, lon, alt, radius, Locale.getDefault().getLanguage(), searchlat, searchlon),
						DataSource.dataFormatFromDataSource(DataSource.DATASOURCE.HOTEL),DataSource.DATASOURCE.HOTEL);}
						
				}
				else if(po==8){
					requestData(DataSource.createRequestURL(DataSource.DATASOURCE.SEARCH, lat, lon, alt, radius, Locale.getDefault().getLanguage(), searchlat, searchlon),
			...
			{
				// 에러가 일어난 상황이지만 재시도 횟수가 1회 이하일 경우
	...
	boolean handleClickEvent(ClickEvent evt) {
		...
		// Handle event
		if (state.nextLStatus == MixState.DONE) {	// 다음상태가 완료일 때(다운로드 완료)
